        ...
        try {
            if (isIdExistOnCustomerTable(lblCustomerId.getText())) {
               ...
               // ResultSet set = CustomerController.getIdData(lblCustomerId.getText());
                CustomerDTO customer = customerBO.getCustomer(lblCustomerId.getText());

                txtName.setText(customer.getFist_name() + " " + customer.getLast_name());
    ...
    private void setItemCodeDetails() {
        try {
            if (lblItemCode.getText().equals("")) {
                ...
                if (itemBo.get(lblItemCode.getText())!=null) {
                    ...
                    ItemsDTO dto = itemBo.get(lblItemCode.getText());
                        txtItemsName.setText(dto.getItem_name());
                        L1:
                        for (int i = 0; i < ids.size(); i++) {
                            if (ids.get(i).getId().equals(lblItemCode.getText())) {
                                txtQty.setText(String.valueOf(dto.getQut() - Integer.parseInt(ids.get(i).getQty())));
                                break L1;
                            } else {
                                txtQty.setText(String.valueOf(dto.getQut()));
                            }
                        }
                        if (ids.isEmpty()) {
                            txtQty.setText(String.valueOf(dto.getQut()));
