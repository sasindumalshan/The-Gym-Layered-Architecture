...
public class EmployeeFromController implements Initializable {
        ...
        try {
           ...
           /* while (ids.next()) {
            ...
            for (EmployeeDTO employeeDTO : all) {
                LoadEmployeeData(employeeDTO.getEmployee_id());
            }
        ...
        try {
            if (search.getText().equals("")) {
                ...
                if (rBtnAllEmployee.isSelected()) {
                 ...
                 ArrayList<String> list=  employeeBO.search(search.getText());
                    for (String id:list) {
                        LoadEmployeeData(id);
                    }
    ...
    private void setAllEmployee() {
        try {
            ...
            txtallEmployee.setText(employeeBO.getEmployeeCount());
        } catch (SQLException | ClassNotFoundException throwables) {
            throwables.printStackTrace();
        }
    }
    ...
    private void searchDate() {
        ...
        try {
            ...
            ArrayList<EmployeeAttendanceProjection> e = employeeBO.getDateForEmployee(dateNow);
            for (EmployeeAttendanceProjection employee:e) {
                selectMonth.setDisable(true);
                FXMLLoader loader = new FXMLLoader(StoreFromController.class.getResource("/lk/ijse/theGym/view/bar/EmployeeAttendaceBar.fxml"));
                Parent root = loader.load();
                EmployeeAttendanceBar controller = loader.getController();
                controller.setData(employee.getEmployeeID(), employee.getFistName(), employee.getLastName(), employee.getEmployeeRoll(), employee.getAttendDate(), employee.getAttendTime());
                vBox.getChildren().add(root);
            }
        } catch (SQLException | ClassNotFoundException | IOException throwables) {
            throwables.printStackTrace();
