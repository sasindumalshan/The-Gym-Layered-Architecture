...
public class OrderController {
  /*  public static ResultSet getFinalTotalOnDay(String day) throws SQLException, ClassNotFoundException {
    ...
    /*public static boolean PlaceOrder(OrderDTO order, ArrayList<lk.ijse.theGym.view.data.Order> orderDetails) throws SQLException {
        Connection connection = null;
        try {
            connection = DBConnection.getInstance().getConnection();
            connection.setAutoCommit(false);
//            OrderController.setOrder(order)
            if (ordersBO.setOrder(order)) {
     ...
     /* private static boolean setOrder(OrderDTO order) throws SQLException, ClassNotFoundException {
